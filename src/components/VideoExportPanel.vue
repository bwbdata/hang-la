...
const props = defineProps<{ mode: 'record' | 'export'; items: VideoItem[]; voiceClips: Record<string, VoiceClip>; introVoice?: VoiceClip; outroVoice?: VoiceClip; ratio: OutputRatio; placementPauseMs: number; generating: boolean; progress: string }>()
const emit = defineEmits<{ saveVoice: [imageId: string, blob: Blob, durationMs: number]; removeVoice: [imageId: string]; saveNarration: [kind: 'intro' | 'outro', blob: Blob, durationMs: number]; removeNarration: [kind: 'intro' | 'outro']; updateRatio: [value: OutputRatio]; updatePause: [value: number]; exportVideo: [] }>()
...
<template>
  <section class="video-panel">
    <div class="video-panel-heading"><div><span class="eyebrow">{{ mode === 'record' ? '录制口播' : '回顾视频' }}</span><h2>{{ mode === 'record' ? '逐张添加口播解说' : '导出排名回顾视频' }}</h2><p>{{ mode === 'record' ? '录音会和对应图片绑定，后续排名时也会一直保留。' : '按最终排名顺序依次展示图片和口播，最后播放完整排名。' }}</p></div><div v-if="mode === 'export'" class="export-video-actions"><div class="export-video-controls"><label>画布比例<select :value="ratio" @change="emit('updateRatio', ($event.target as HTMLSelectElement).value as OutputRatio)"><option value="16:9">16 : 9</option><option value="3:4">3 : 4</option></select></label><label>放入后停留<select :value="placementPauseMs / 1000" @change="emit('updatePause', Number(($event.target as HTMLSelectElement).value) * 1000)"><option :value="0">不停止</option><option :value="1">1 秒</option><option :value="1.5">1.5 秒</option><option :value="2">2 秒</option><option :value="3">3 秒</option></select></label><button type="button" class="primary-button" :disabled="generating || !items.length" @click="emit('exportVideo')">{{ generating ? progress : '导出 WebM 视频' }}</button></div><small>比例决定视频成片尺寸；停留时长用于突出图片入榜后的最终名次。</small></div></div>
    <div v-if="mode === 'record'" class="narration-recorders"><article><div><b>开头解说</b><small>背景为尚未放入图片的空榜单</small></div><VoiceRecorder :clip="introVoice" @save="emit('saveNarration', 'intro', $event.blob, $event.durationMs)" @remove="emit('removeNarration', 'intro')" /></article><article><div><b>结尾解说</b><small>背景为全部图片放入后的最终榜单</small></div><VoiceRecorder :clip="outroVoice" @save="emit('saveNarration', 'outro', $event.blob, $event.durationMs)" @remove="emit('removeNarration', 'outro')" /></article></div>
