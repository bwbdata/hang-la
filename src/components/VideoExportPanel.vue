...
<template>
  <section class="video-panel">
    <div class="video-panel-heading"><div><span class="eyebrow">{{ mode === 'record' ? '录制口播' : '回顾视频' }}</span><h2>{{ mode === 'record' ? '逐张添加口播解说' : '导出排名回顾视频' }}</h2><p>{{ mode === 'record' ? '录音会和对应图片绑定，后续排名时也会一直保留。' : '可在此页补录或修改片头、片尾及每张图片的口播，再直接导出。' }}</p></div><div v-if="mode === 'export'" class="export-video-actions"><div class="export-video-controls"><label>画布比例<select :value="ratio" @change="emit('updateRatio', ($event.target as HTMLSelectElement).value as OutputRatio)"><option value="16:9">16 : 9</option><option value="3:4">3 : 4</option></select></label><label>放入后停留<select :value="placementPauseMs / 1000" @change="emit('updatePause', Number(($event.target as HTMLSelectElement).value) * 1000)"><option :value="0">不停止</option><option :value="1">1 秒</option><option :value="1.5">1.5 秒</option><option :value="2">2 秒</option><option :value="3">3 秒</option></select></label><label>视频格式<select :value="format" @change="emit('updateFormat', ($event.target as HTMLSelectElement).value as 'webm' | 'mp4')"><option value="webm">WebM（推荐）</option><option value="mp4">MP4（首次约加载 30MB）</option></select></label><button type="button" class="primary-button" :disabled="generating || !items.length" @click="format === 'mp4' ? emit('exportMp4') : emit('exportVideo')">{{ generating ? progress : `导出 ${format.toUpperCase()} 视频` }}</button></div><small>WebM 直接由浏览器生成；MP4 需额外加载约 30MB 本地转换引擎。比例决定成片尺寸，停留时长突出最终名次。</small></div></div>
