...
async function exportVideo() {
  ...
  try {
    const blob = await exportRecapVideo({ title: ranking.draft.title || '夯拉排名', ratio: ranking.draft.aspectRatio, tierNames: ranking.draft.tiers.map((tier) => tier.name), items: videoItems.value, voiceClips: ranking.draft.voiceClips, onProgress: (message) => { videoProgress.value = message } })
...
<template>
  <main v-if="ranking.isReady" class="app-shell">
    ...
    <template v-else-if="ranking.step === 3">
      <VideoExportPanel mode="record" :items="Object.values(ranking.draft.images).map((image) => ({ imageId: image.id, name: image.name, url: image.previewUrl, tierName: '' }))" :voice-clips="ranking.draft.voiceClips" :generating="false" progress="" @save-voice="ranking.saveVoiceClip" @remove-voice="ranking.removeVoiceClip" @export-video="exportVideo" />
    ...
    <template v-else>
      ...
      />
      <VideoExportPanel v-if="ranking.step === 5" mode="export" :items="videoItems" :voice-clips="ranking.draft.voiceClips" :generating="videoGenerating" :progress="videoProgress" @save-voice="ranking.saveVoiceClip" @remove-voice="ranking.removeVoiceClip" @export-video="exportVideo" />
