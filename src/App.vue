...
<template>
  <main v-if="ranking.isReady" class="app-shell">
    ...
    <template v-else-if="ranking.step === 4">
      ...
      />
      <footer class="step-footer"><button class="secondary-button" type="button" @click="previousStep">← 上一步</button><button class="secondary-button" type="button" :disabled="!ranking.canExport || pngGenerating" @click="exportImage">{{ pngGenerating ? '正在生成 PNG…' : '导出 PNG' }}</button><button class="primary-button" type="button" :disabled="!ranking.canExport" @click="nextStep">下一步：导出视频 <span>→</span></button></footer>
    </template>
    <template v-else>
      ...
      <footer class="step-footer"><button class="secondary-button" type="button" @click="previousStep">← 返回排名</button></footer>
    </template>
  </main>
